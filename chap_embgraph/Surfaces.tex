...
\examplefigure{Every planar graph embedding gives us an example of a surface embedding. A graph which is embeddable in the plane is embeddable in every surface, as each surface locally looks like a copy of the plane. Because of this, we see that one requires more than just the information of a graph on a surface to determine what surface we are looking at. }{\begin{tikzpicture}[scale=.18]

\draw  (-3,-0.5) ellipse (8 and 3);
\draw  plot[smooth, tension=.7] coordinates {(-6,0) (-5,-1) (-3,-1.5) (-1,-1) (0,0)};
\draw  plot[smooth, tension=.7] coordinates {(-5,-1) (-3,-0.5) (-1,-1)};
\draw (-2.5,8) -- (-3.5,6) -- (-1.5,6) -- (-2.5,8);
\draw (-5.5,5) -- (-3.5,6);
\fill(-2.5,8) circle[radius=.2];
\fill(-9,-2) circle[radius=.2];
\fill(-7,0) circle[radius=.2];
\fill(-6.8,-1.6) circle[radius=.2];
\fill(-8.4,-0.8) circle[radius=.2];
\fill(-5.5,5) circle[radius=.2];
\fill(-1.5,6) circle[radius=.2];
\fill(-3.5,6) circle[radius=.2];
\draw  plot[smooth, tension=.7] coordinates {(-7,0) (-8,0) (-8.4,-0.8)};
\draw  plot[smooth, tension=.7] coordinates {(-8.4,-0.8) (-7.8,-1.6) (-6.8,-1.6) (-6.4,-1) (-7,0)};
\draw  plot[smooth, tension=.7] coordinates {(-8.4,-0.8) (-9.2,-1.2) (-9,-1.8)};
\end{tikzpicture}}

\examplefigure{Graphs which are embeddable in the sphere are exactly equal to planar graphs. One can translate between these two types of graph via a technique called \emph{stereographic projection.}
...
\examplefigure{There are of some more intesting graphs that we can embed on the torus. In the figure on the right, the graph drawn has only 1 face; but any planar drawing of this graph will have 3 faces as the graph has 2 cycles. }{\begin{tikzpicture}[scale=.18]
\draw  (-3,-0.5) ellipse (8 and 3);
\draw  plot[smooth, tension=.7] coordinates {(-6,0) (-5,-1) (-3,-1.5) (-1,-1) (0,0)};
\draw  plot[smooth, tension=.7] coordinates {(-5,-1) (-3,-0.5) (-1,-1)};
\fill(-5.2,-2.2) circle[radius=.3];
\draw  plot[smooth cycle, tension=.7] coordinates {(-3,-2.5) (-8.5,-0.5) (-3,1.5) (2.5,-0.5)};
\draw  plot[smooth, tension=1] coordinates {(-3.8,-1.4) (-5.2,-2.2) (-4.4,-3.4)};
\draw[dashed]  plot[smooth, tension=1] coordinates {(-3.8,-1.4) (-3.6,-2.4) (-4.4,-3.4)};
\end{tikzpicture}}
...
\examplefigure{The torus has a nice description via a gluing polygon. When you take the sides labeled $a$ and glue them together, you get a cylinder. Then taking the top and bottom of the cylinder (labeled $b$) and bring them together folds this into a torus. }{\begin{tikzpicture}[scale=.26]
...
\[5-6+1=0.\]}{\begin{tikzpicture}[scale=.5]
\draw  (-2,2) rectangle (2,-2);
\draw (-2.2,0.4) -- (-2,0.2) -- (-1.8,0.4);
\draw (-2.2,0.2) -- (-2,0) -- (-1.8,0.2);
\draw (1.8,0.4) -- (2,0.2) -- (2.2,0.4);
...
While this map is not injective, it is a valid map of surfaces. }{\begin{tikzpicture}[scale=.4]
...
\end{align*} }{\begin{tikzpicture}[scale=.5]
...
\begin{claim}
	There always exists a Morse function $f: \Sigma\to \R$.
\end{claim}
\begin{proof}
...
\end{tikzpicture}}
\begin{theorem}
	Suppose $f: \Delta(\Sigma) \to \R$  be Morse function. Then 
...
\examplefigure{Notice in this net for the torus, there is only 1 face. However, there are 2 cycles: the one that goes meridinally around the torus, and the one that goes longitudinally. Since there are fewer faces than cycles, it is clear that they do not form a good basis for the cycle space.}{\begin{tikzpicture}[scale=.4]
